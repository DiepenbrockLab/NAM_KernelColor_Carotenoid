...
consolidated.GWAS.results.path = paste(home.dir, "/GWASResults_Upliftable/Summaries_by_Trait/",sep='')
...
for(i in trait){
  #Note to Mishi 1/14/2020: will need to change each of these to reflect the number assigned to the subfolder within each trait's GWAS results folder 
  if(i == "a_carotene") folder = "-1582910345/"
  if(i == "b_carotene") folder = "-1582910686/"
  if(i == "b_cryp") folder = "-1582910693/"
  if(i == "lutein") folder = "-1583382876/"
  if(i == "phytofluene") folder = "-1583382882/"
  if(i == "total_carot") folder = "-1584058759/"
  if(i == "zeaxanthin") folder = "-1584058764/"
  if(i == "zeino") folder = "-1584058769/"
  if(i == 'traitKC') folder = "-1583382891/"
  
  raw.GWAS.results.path = paste(home.dir, "/GWASResults_Upliftable/",i,"_",folder,sep='') #CHD changed / to _ for new upliftable folder name
...
#Read in the ten files, and summarize their counts
     for(j in 1:10){
   #for(j in 1:num.chr){
      #Read in the data - note to Mishi 1/14/2020: ill need to adjust these numbers based on how many breaks between 0 and 100 the script/server is making
